        ...
        for i in self.list:
            ...
            for j in self.list:
                ...
                if i.coll==1 and j.coll==1:
                    if abs(x1-x2)<rad1+rad2 and abs(y1-y2)<rad1+rad2:
                        i.coll=0
                        j.coll=0
        ...
        for i in range(len(self.time_array) - 1):
            ...
            if coll:
                ...
                for i in seznam1:
                    ...
                    v1xr=((m1-m2)*v1x+2*m2*v2x)/(m1+m2)
                    v2xr = ((m2 - m1)*v2x+2 * m1 * v1x) / (m1 + m2)
...
if __name__ == '__main__':
    ...
    b3 = Ball(3, 27, v_0 * np.cos(alpha_2), H*2, v_0 * np.sin(alpha_1), 0, 0, radius, 1, [])
    #b1 = Ball(1, 0, v_0 * np.cos(alpha_0), H * 2, v_0 * np.sin(alpha_0), 0, 0, radius, 1, [])
    #b2 = Ball(2, 0, v_0 * np.cos(alpha_1), H , v_0 * np.sin(alpha_0), 0, 0, radius, 1, [])
