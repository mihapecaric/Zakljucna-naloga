    ...
    for i in range(len(time_array) - 1):
        if ball.get_motion_status() == 'projectile_motion':
            ...
            if abs(ball.get_velocity_x()) - abs(ball.get_velocity_phi()) * ball.get_radius() < 0.1:
                ...
                distance_phi = (5 * mi * g) / (4 * ball.get_radius()) * (time_array[i]- ball.get_time_after_collision())**2
...
if __name__ == '__main__':
   ...
   # ax.add_patch(ball)
    ...
    #print(f'Distance_phi after sliding = {b1.get_distance_phi_after_sliding(): .2f} rad.')
    ...
    print(f'čas po odbojih zračunan s self:{b1.get_time_after_collision()} s')
    print(f'čas po sliding zračunan s self:{b1.get_time_after_sliding()} s')
    
"""def time_solution(ball):
    values_array = []
    #state=get_state(ball)
    state = ball.get_motion_status()
